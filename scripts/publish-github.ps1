...
$Tag='v0.1.2-alpha'
$ExpectedHash='6904138b707baadd825c6a901b2a83e5511390b202cc06f733820730b4e467c4'
...
$Tarball=(Get-ChildItem -LiteralPath release -Filter 'flop-tools-conformance-lab-0.1.2.tgz' -File).FullName
...
if(-not$ExistingTag){git tag -a $Tag -m 'FLOP Conformance Lab v0.1.2-alpha'}
git push origin $Tag
gh release view $Tag --repo "$Owner/$RepoName" 1>$null 2>$null
if($LASTEXITCODE-ne 0){gh release create $Tag $Tarball release\SHA256SUMS release\example-report.json conformance\sources\manifest.json --repo "$Owner/$RepoName" --title 'FLOP Conformance Lab v0.1.2-alpha' --notes-file RELEASE_NOTES.md --prerelease}else{gh release upload $Tag $Tarball release\SHA256SUMS release\example-report.json conformance\sources\manifest.json --repo "$Owner/$RepoName" --clobber}
